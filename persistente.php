...
abstract class Persistent{
  ...
  */
  final protected function getFields(array $field_names=null){
    ...
    if (isset($field_names))
      $sql = sprintf("SELECT %s FROM %s WHERE id = %s", implode(',', $field_names), $this->getTableName(), $this->id);
    else {
      $sql = sprintf("SELECT * FROM %s WHERE id  = %s", $this->getTableName(), $this->id);
    }
    $result = $this->db->query($sql);
    if (!isset($result)) {
  ...
  */
  final protected function setFields(array $field_values){
    ...
    $sql = sprintf("UPDATE %s SET %s WHERE id = %s", $this->getTableName(), implode(", ", $s) , $this->id);
    $result = $this->db->query($sql);
    return $result;
  }
  ...
  final function delete(){
    ...
    $sql = array();
    $sql[] = sprintf("DELETE FROM %s WHERE id  = %s", $this->getTableName(), $this->id);
